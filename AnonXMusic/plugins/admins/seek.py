...
async def seek_comm(cli, message: Message, _, chat_id):
    if len(message.command) == 1:
        mystic = await message.reply_text(_["admin_20"])
        await delete_message(message.chat.id, mystic.id)
        return 
    query = message.text.split(None, 1)[1].strip()
    if not query.isnumeric():
        mystic = await message.reply_text(_["admin_21"])
        await delete_message(message.chat.id, mystic.id)
        return 
    ...
    if duration_seconds == 0:
        mystic = await message.reply_text(_["admin_22"])
        await delete_message(message.chat.id, mystic.id)
        return
    ...
    if "vid_" in file_path:
        ...
        if n == 0:
            to_del = await message.reply_text(_["admin_22"])
    ...
    try:
        ...
        await mystic.edit_text(_["admin_26"], reply_markup=close_markup(_))
        await delete_message(message.chat.id, mystic.id)
        return
    if message.command[0][-2] == "c":
