    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            if "track" in url:
                ...
                await mystic.edit_text(_["play_15"])
                await delete_message(chat_id, mystic.id)
                return
...
async def anonymous_check(client, CallbackQuery):
    try:
        mystic = await CallbackQuery.answer(
...
async def slider_queries(client, CallbackQuery, _):
    ...
    if what == "F":
        ...
        await delete_message(CallbackQuery.message.chat.id, mystic.id, long_seconds=30)
        return
    if what == "B":
        if rtype == 0:
            query_type = 9
